...
void
SerialFilPiloteSendCommande (char cmd)
{
  write (fd_fil_pilote, &cmd, 1);
}

void*
uart_filPilote_loop (void *arg)
{
  char str[255];
  char buf[255];
  int index = 0;
      ...
      for (int ii = 0; ii < sizeof(buf); ii++)
	{
	  if ((buf[ii] == '\n') || (buf[ii] == '\r'))
	      ...
	      if (buf[1] == 'C')
		{
		  ...
		  buf[16] = 0;
		  printf ("%s\n", buf);
		  //BD float temperature = strtol(buf + 13, 0, 16) / 10.0f;
		  //BD buf[10]=0;
		  //C:6503804 17.50
		  //BD sprintf(bufpipe,"%s %2.2f\n", &buf[1], temperature);
		  //BD sprintf(bufpipe,"%s %s", &buf[1],  &buf[13]);
		  printf ("%s\n", bufpipe);
		  sem_wait (&semaphore);
		  write (pipe, &buf[1], 15);
		  sem_post (&semaphore);
